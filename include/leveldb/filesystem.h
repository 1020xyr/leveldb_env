...
using FileInfo = std::pair<std::string, FileType>;

class Directory {
 public:
  Directory() = default;
  void AddChildren(FileInfo info) { childrens_.emplace_back(info); }  // 添加子文件
...
class SimpleFileSystem {
 public:
  ...
  bool FileExists(const std::string& fname) { return files_.count(fname) > 0; }
