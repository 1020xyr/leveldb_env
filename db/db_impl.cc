...
Status DBImpl::InstallCompactionResults(CompactionState* compact) {
  mutex_.AssertHeld();
  Log(options_.info_log, "Compacted %d@%d + %d@%d files => %lld bytes", compact->compaction->num_input_files(0), compact->compaction->level(), compact->compaction->num_input_files(1), compact->compaction->level() + 1, static_cast<long long>(compact->total_bytes));
...
Status DBImpl::Write(const WriteOptions& options, WriteBatch* updates) {
  ...
  while (!w.done && &w != writers_.front()) {  // 写入完成或为写者队列队首
    w.cv.Wait();
  }
  ...
  if (status.ok() && updates != nullptr) {                            // nullptr batch is for compactions
    ...
    {
      mutex_.Unlock();                                                      // 释放互斥锁
      status = log_->AddRecord(WriteBatchInternal::Contents(write_batch));  // 写入日志
  ...
  if (in.starts_with("num-files-at-level")) {
    ...
    for (int level = 0; level < config::kNumLevels; level++) {
      ...
      if (stats_[level].micros > 0 || files > 0) {
        std::snprintf(buf, sizeof(buf), "%3d %8d %8.0f %9.0f %8.0f %9.0f\n", level, files, versions_->NumLevelBytes(level) / 1048576.0, stats_[level].micros / 1e6, stats_[level].bytes_read / 1048576.0, stats_[level].bytes_written / 1048576.0);
        value->append(buf);
      }
    }
    return true;
